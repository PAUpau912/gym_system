...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['submit'])) {
    ...
    $stmt = $conn->prepare("update member SET FIRSTNAME=?,LASTNAME=?, gender=?,birthday=?,height=?,weight=?,contact=?,address=?,joining_date=?,image=? 
    WHERE user_id=? ");
    $stmt->bind_param("ssssiiissss",$first_name, $last_name, $gender,$birthday,$height,$weight,$number,$address,$join, $imageData,$user_id);
        ...
        <div class="info-container">
            ...
            <div class="row">
                ...
                <div class="col-sm-12 col-md-3">
                    ...
                    <input type="number" name="height" class="form-control-md" placeholder="cm" maxlength="3" min="55"
                        max="500" required>
                </div>

                <div class="col-sm-12 col-md-3">
                    <label for="exampleFormControlInput1" class="form-label">Width</label>
                    <input type="number" name="weight" class="form-control-md" maxlength="2" placeholder="in kg"
                        min="44" max="150" required>
                </div>
            </div>
